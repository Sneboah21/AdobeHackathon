...
Rounds 1A & 1B Implementation
Performance: <3 seconds for 50-page PDF
Enhanced with USER CHOICE for round selection
"""
...
def get_user_choice() -> str:
    """
    NEW: Interactive user choice for round selection
    Returns: '1a', '1b', 'both', or 'quit'
    """
    print("\n" + "=" * 60)
    print("🎯 ADOBE HACKATHON - ROUND SELECTION")
    print("=" * 60)
    print("Choose which round(s) you want to execute:")
    print()
    print("1️⃣  Round 1A Only  - Document Structure Extraction")
    print("2️⃣  Round 1B Only  - Persona-Driven Document Intelligence")
    print("3️⃣  Both Rounds    - Complete Solution")
    print("4️⃣  Quit          - Exit application")
    print()
    print("=" * 60)
...
def process_round_1b(input_dir: Path, output_dir: Path) -> bool:
    ...
    config_file = input_dir / "persona_config.json"
    if not config_file.exists():
        print("❌ No persona_config.json found!")
        print("💡 To run Round 1B, create a persona_config.json file with:")
        print('   {"persona": "Your persona", "job_to_be_done": "Your job description"}')
        return False
...
def main():
    """
    Main execution function - ENHANCED with user choice functionality
    Orchestrates Round 1A and/or 1B processing based on user selection
    """
    print("=" * 70)
    print("🏆 Adobe Hackathon - Elite Document Intelligence Solution")
    print("🎯 Rounds 1A & 1B - Structure Extraction & Persona Analysis")
    ...
    # NEW: Get user choice for round selection
    ...
    try:
        if user_choice in ['1a', 'both']:
            success_1a = process_round_1a(input_dir, output_dir)
            if success_1a:
                overall_success = True

        if user_choice in ['1b', 'both']:
            success_1b = process_round_1b(input_dir, output_dir)
            if success_1b:
                overall_success = True
    ...
    print("📊 FINAL EXECUTION SUMMARY")
    print("=" * 70)
    print(f"🎯 User Selection: {user_choice.upper()}")
    print(f"⏱️  Total execution time: {total_time:.2f}s")
    ...
    if overall_success:
        ...
        # Provide specific feedback about performance
        if user_choice in ['1a', 'both'] and total_time <= 10:
            print("🎯 Solution meets all performance requirements!")
    else:
        print("\n❌ SOLUTION FAILED")
        print("📝 Check error messages above and output directory for debug info")
